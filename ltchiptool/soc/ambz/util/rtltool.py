...
def main(*argv):
    ...
    subparsers.add_parser("gf", help="Get Flash Status register")

    subparsers.add_parser("sf", help="Set Flash Status register")
    ...
    if rtl.connect():
        if args.operation == "wf":
            ...
            if size < 1:
                stream.close()
                raise RuntimeError("File size = 0!")
        ...
        # 			sys.exit(0)
            ...
            if rtl.EraseSectorsFlash(offset, size):
                print("Done!")
                sys.exit(0)
            raise RuntimeError("Erase Flash sectors!")
            ...
            stream = io.BytesIO(
                b"\x19\x20\x00\x10\x19\x20\x00\x10"
                b"\x19\x20\x00\x10\x19\x20\x00\x10"
                b"\x19\x20\x00\x10\x00\x00\x00\x00"
        ...
        raise RuntimeError("Failed to connect device on", args.port, "!")

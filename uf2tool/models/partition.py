...
from datastruct.fields import const, field, padding, string, subfield, varlist
...
class Partition(DataStruct):
    ...
    name: str = string(lambda ctx: ctx.G.root.name_len or 24)
    flash_name: str = string(lambda ctx: ctx.G.root.name_len or 24)
    offset: int = field("I")
    length: int = field("I")
    _1: ... = padding(4)
